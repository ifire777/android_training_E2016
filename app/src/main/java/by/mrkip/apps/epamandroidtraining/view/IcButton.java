...
public class IcButton extends Button {
	...
	private void init(final AttributeSet attrs, final int defStyleAttr) {
		...
		try {
			...
			Drawable downIcon = a.getDrawableIfKnown(R.styleable.IcButtonStyle_underIcon);
			id = a.getResourceId(R.styleable.IcButtonStyle_underIcon, -1);
			if (id != -1) {
				downIcon = drawableManager.getDrawable(getContext(), id);
			}
			...
			setCompoundDrawablesWithIntrinsicBounds(leftIcon, topIcon, rightIcon, downIcon);
